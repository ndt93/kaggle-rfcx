...
def do_self_mixup(x, mixup_lambda, mixup_indices):
    """Mixup x of even indexes (0, 2, 4, ...) with x of odd indexes 
    (1, 3, 5, ...).
    ...
    Args:
      ...
      mixup_indices: (batch_size,)

    Returns:
      out: (batch_size, ...)
    """
    a = x.transpose(0, -1) * mixup_lambda
    b = x[mixup_indices].transpose(0, -1) * (1 - mixup_lambda)
    out = (a + b).transpose(0, -1)
    return out
