...
from pytorch_utils import do_mixup, interpolate, pad_framewise_output, do_self_mixup
...
class Cnn14_DecisionLevelAtt(nn.Module):
    def __init__(self, sample_rate, window_size, hop_size, mel_bins, fmin, 
        ...
        # Spec augmenter
        self.spec_augmenter = SpecAugmentation(time_drop_width=88, time_stripes_num=2, 
            freq_drop_width=5, freq_stripes_num=4)
    ...
    def forward(self, input, mixup_lambda=None, mixup_indices=None):
        """
        Input: (batch_size, data_length)"""
        ...
        if self.training and np.random.uniform() < 0.5:
        ...
        if self.training and np.random.uniform() < 0.7:
            x = self.spec_augmenter(x)

        # Mixup on spectrogram
        if self.training and mixup_lambda is not None:
            x = do_self_mixup(x, mixup_lambda, mixup_indices)
